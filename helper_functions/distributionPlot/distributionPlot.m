        ...
        switch floor(opt.histOpt)
            ...
            case 1
                ...
                if opt.histOpt == 1.1
                    ...
                    if opt.divFactor ~= 1
                        [xHist,yHist] = ksdensity(currentData,'kernel','normal','width',u/opt.divFactor,'NumPoints',1000);
                    end
                end
    ...
    switch opt.xyOri
        case 'normal'
            ...
            if opt.showMM == 6
                ...
                mdh(2,:) = plot(ah,squeeze(md(:,2,2:3))',repmat(md(:,2,1)',2,1),'color','b','lineWidth',1);%,'lineStyle','--');
                mdh(3,:) = plot(ah,squeeze(md(:,3,2:3))',repmat(md(:,3,1)',2,1),'color','b','lineWidth',1);%,'lineStyle','--');
            end
        case 'flipped'
            if any(opt.showMM==[1,2])
